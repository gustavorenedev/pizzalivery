...
export default function Summary() {
  ...
  const { setPizzaOrder, chosenPizzas, setPizzaSize, setPizzaFlavour } =
          ...
          return (
            <SummaryDetails key={index}>
              ...
              <SummaryTitle>
                {pizza.flavours
                  .map((item) => item.name)
                  .join(pizza.flavours.length > 1 ? " e " : "")}
              </SummaryTitle>
              <SummaryDescription>
                {pizza.size.text} {`(${pizza.size.slices}) pedaços`}
              </SummaryDescription>
              <SummaryPrice>
                {convertToCurrency(pizza.flavours[0].price[pizza.size.slices])}
              </SummaryPrice>
            </SummaryDetails>
          );
        })}
